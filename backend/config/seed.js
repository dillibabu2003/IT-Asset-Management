...
const seedDB = async () => {
  ...
  for (let i = 3; i <= 100; i++) {
    ...
    users.push(
      new User({
        user_id:
          i < 10
            ? `DBOX${role}${i.toString().padStart(1, "0")}`
            : `DBOX${role}${i}`,
        role: role,
  ...
  for (let i = 1; i <= 20; i++) {
    const status =
      licenseStatuses[Math.floor(Math.random() * licenseStatuses.length)];
    licenses.push(
      new License({
        serial_no: `SN${i.toString().padStart(3, "0")}`,
        license_id:
  ...
  const dashboards = [
    {
      ...
      elements: [
        ...
        },
        {
          ...
          query:
            '[{"$sort":{"updatedAt":-1}},{"$project":{"serial_no":1,"asset_id":1,"ram":1,"storage":1,"processor":1,"model":1,"status":1}},{"$limit":6}]',
        },
      ],
    },
    {
      ...
      elements: [
        ...
        },
        {
          ...
          type: "table",
          fields: ["license_id", "date_of_received", "warranty", "status"],
          query:
            '[{"$sort":{"updatedAt":-1}},{"$project":{"license_id":1,"date_of_received":1,"warranty":1,"status":1}},{"$limit":6}]',
        },
      ],
    },
    {
      ...
      tiles: [
        ...
        },
        {
          ...
          color: "#29ac20",
          query:
            '[{"$match":{"status":"pending"}},{"$group":{"_id":null,"value":{"$sum":{"$toDecimal":"$amount"}}}},{"$project":{"_id":0,"value":{"$toString":"$value"}}}]',
        },
      ],
      elements: [
        {
        ...
        },
        {
          ...
          query:
            '[{"$sort":{"updatedAt":-1}},{"$project":{"invoice_id":1,"date_of_received":1,"date_of_upload":1,"amount":1,"status":1}},{"$limit":6}]',
        },
      ],
    },
  ];
  ...
  await MetaData.insertMany([
    ...
    },
    {
      ...
      options: genderEnum.map((gender) => ({
        label: gender.substring(0, 1).toUpperCase() + gender.substring(1),
        value: gender,
      })),
    },
  ]);

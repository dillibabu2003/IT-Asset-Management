    ...
    virtuals: {
      ...
      name: {
        get() {
          ...
          const model =
            this.model.charAt(0).toUpperCase() + this.model.substring(1);
...
LicenseSchema.methods.generateId = function (
  employeeId,
  ...
  if (
    ...
    totalCheckoutsTillNowOfEmployee === undefined
  ) {
  ...
  if (docStatus !== "available" && docStatus !== "reissue") {
    throw new Error(
      "Cannot generate license id for non-available or non-reissue licenses",
    );
  }
...
LicenseSchema.statics.updateLicenseStatuses = async function (
  daysBeforeExpiry = 7,
) {
  const now = new Date();

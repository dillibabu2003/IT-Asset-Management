...
AssetSchema.methods.generateId = function (
  employeeId,
  ...
  if (docStatus !== "available" && docStatus !== "reissue") {
    throw new Error(
      "Cannot generate asset id for non-available or non-reissue assets",
    );
  }
...
AssetSchema.statics.updateAssetStatuses = async function (
  daysBeforeExpiry = 7,
) {
  const now = new Date();

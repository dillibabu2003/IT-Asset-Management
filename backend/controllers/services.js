...
const generatePutObjectUrl = asyncHandler(async (req, res) => {
    ...
    if(!type || !content_type || !file_name){
        throw new ApiError(400,null,"Type of file i.e. profile-pic or invoice, file name and content type are required");
    }
...
const generateGetObjectUrl = asyncHandler(async (req, res) => {
    ...
    const response = await createGetObjectPreSignedURL(cleanedEnv.S3_BUCKET_NAME, cleanedEnv.S3_INVOICES_FOLDER+"/"+key);
    if(response.error){
        throw new ApiError(500,null,response.error);
    }

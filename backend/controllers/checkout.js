...
async function executeAssignItemsCheckoutTransaction(
  model,
  serial_numbers,
  objectName,
  employees_info,
  ...
  try {
    ...
    const availableItems = await validateAvailabilityAndReturnItems(
      model,
      serial_numbers,
    );
    ...
    const entireCheckoutCountTillNow = await Checkout.countDocuments(
      {},
      { session: session },
    );
    for (let i = 0; i < availableItems.length; i++) {
      ...
      availableItems[i].generateId(
        employeeDetails.employee_id,
        totalCheckoutsTillNowOfEmployee,
      );
      availableItems[i].status =
        objectName == "assets" ? "deployed" : "activated";
      ...
      const checkout = createCheckout(
        objectName,
        employeeDetails,
        availableItems[i]._id,
        entireCheckoutCountTillNow + i + 1,
      );
...
async function executeUnassignItemsCheckoutTransaction(
  model,
  objectName,
  infoToUnassign,
) {
  ...
  try {
    ...
    for (let i = 0; i < infoToUnassign.length; i++) {
      const employeeDetails = await validateEmployeeAndReturnEmployeeDetails(
        infoToUnassign[i].employee_id,
      );
      const item = await model.findOne({
        assigned_to: employeeDetails._id,
        serial_no: infoToUnassign[i].serial_no,
      });
      if (!item) {
        throw new ApiError(
          400,
          null,
          `Item with serial number ${infoToUnassign[i].serial_no} not found or not assigned to this employee`,
        );
      }
...
const assignBulkItems = asyncHandler(async (req, res) => {
  ...
  const employee_ids = info_to_assign.map((item) => {
    return { employee_id: item.employee_id };
  });
...
const unAssignItem = asyncHandler(async (req, res) => {
  ...
  if (
    !object_name ||
    typeof info_to_unassign != "object" ||
  ...
  await executeUnassignItemsCheckoutTransaction(model, object_name, [
    info_to_unassign,
  ]);
...
const unAssignBulkItems = asyncHandler(async (req, res) => {
  ...
  await executeUnassignItemsCheckoutTransaction(
    model,
    object_name,
    info_to_unassign,
  );
...
const getPaginatedCheckouts = asyncHandler(async (req, res) => {
  ...
  res
    ...
    .json(
      new ApiResponse(
        200,
        { checkouts: [...checkouts], total: totalCheckouts },
        "Checkouts fetched successfully",
      ),
    );
});
module.exports = {

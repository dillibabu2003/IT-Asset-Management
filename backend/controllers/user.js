...
const initailizeAllObjectsFieldsAsVisibleWithUserid = async (
  userId,
  session,
) => {
...
const createUser = asyncHandler(async (req, res) => {
  ...
  const { emailResponse, error } = await hitEmailServerApi(
    cleanedEnv.EMAIL_API,
...
const getUserProfile = asyncHandler(async (req, res) => {
  ...
  res
    .status(200)
    .json(
      new ApiResponse(200, filteredUserInfo, "User data fetched Successfully"),
    );
});

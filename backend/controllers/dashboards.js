...
function validateTilesTarget(id, target, func) {
  ...
  return (
    validTypes.includes(schemaField.instance) &&
    (func === "sum" || func === "avg")
  );
}
function validateElementFields(id, fields, type) {
  ...
  if (type === "pie" || type === "bar") {
    ...
    const schemaField = model.schema.path(field);
    return (
      schemaField && schemaField.enumValues && schemaField.enumValues.length > 0
    );
  }
...
function validateConfigureDashboardTiles(id, tiles) {
  tiles.forEach((tile) => {
    ...
    if (
      !validateTilesMatcherValue(id, tile.matcher_field, tile.matcher_value)
    ) {
      return false;
    }
...
function getTilesWithQueriesAttached(tiles) {
  tiles.forEach((tile) => {
    ...
    if (tile.func == "count") {
      query =
        tile.matcher_value != ""
          ? constructCountQuery(tile.matcher_field, tile.matcher_value)
          : constructCountAllQuery();
    } else if (tile.func == "sum") {
      query = constructSumQuery(
        tile.matcher_field,
        tile.matcher_value,
        tile.target,
      );
    } else {
      query = constructAverageQuery(
        tile.matcher_field,
        tile.matcher_value,
        tile.target,
      );
...
const handleGetDashboardData = asyncHandler(async (req, res) => {
  ...
  const parsedDashboardData = await parseRawDashboardData(
    rawDashboardData,
    model,
  );
  res
  ...
  res
    ...
    .json(
      new ApiResponse(
        200,
        dashboardMetaData,
        "Dashboard metadata fetched successfully",
      ),
    );
});
...
const handleConfigureDashboard = asyncHandler(async (req, res) => {
  ...
  const areElementsValid = validateConfigureDashboardElements(
    data.id,
    data.elements,
  );
  if (!areTilesValid || !areElementsValid) {
    throw new ApiError(
      ...
      null,
      !areTilesValid
        ? "Some tiles info are incorrect."
        : "Some elements info are incorrect.",
    );
  }
  ...
  const newDashboardDocument = await Dashboard.findOneAndUpdate(
    { id: data.id },
    data,

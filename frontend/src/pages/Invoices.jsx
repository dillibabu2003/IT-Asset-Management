...
const InvoicesPage = () => {
    ...
    const [deleteDialogInfo, setDeleteDialogInfo] = useState({showDialog: false, invoice_id: null});
    ...
    async function fetchInvoicesMetaData(abortController) {
        const response = await axiosInstance.get("/metadata/invoices", { signal: abortController.signal });
        return response.data;
    }
    ...
    async function refreshData() {
        ...
        fetchInvoicesByPageAndLimit(abortController,page,pageLimit).then((response) => {
            ...
            setData(prev=>{return {...prev, data: invoices }});
        }).catch((error) => {
            console.log(error);
    ...
    async function deleteInvoice(){
        try {
            ...
            if(response.data.success){
                ...
                setDeleteDialogInfo(prev=>{return {...prev, showDialog: false}});
                setTimeout(() => {
                    refreshData();
                }
                , 1000);
        ...
        !data ? <Loader /> :
            ...
            <Box sx={{ p: 3 }}>
                ...
                <Dialog
                    ...
                    onClose={() => setDeleteDialogInfo(prev => { return { ...prev, showDialog: false } })}
                >
                    <DialogTitle>Confirm Delete</DialogTitle>
                    <DialogContent>
                                Are you sure you want to delete the invoice {deleteDialogInfo.invoice_id}?
                            <Typography variant="body2" sx={{ mt: 2, color: "text.secondary" }}>
                                This action cannot be reverted, all the licenses and assets belong to this invoice will be deleted.
                            </Typography>
                    </DialogContent>
                    <DialogActions>
                        <Button onClick={() => setDeleteDialogInfo(prev => { return { ...prev, showDialog: false } })}>Cancel</Button>
                        <Button onClick={deleteInvoice} color="error">Delete</Button>
                    </DialogActions>
                </Dialog>

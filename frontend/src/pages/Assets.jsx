...
const AssetsPage = () => {
  ...
  const [filters, setFilters] = useState(
    sessionStorage.getItem("assets-filters")
      ? JSON.parse(sessionStorage.getItem("assets-filters"))
      : null,
  );
  //Fetching Data Functions
  const fetchAssetsByPageAndLimit = useCallback(
    async (abortController, page, limit) => {
      const response = await axiosInstance.get(
        "/objects/assets/filter-docs/paginate",
        {
          params: {
            page: page,
            limit: limit,
  ...
  async function fetchAssetsMetaData(abortController) {
    const response = await axiosInstance.get("/metadata/assets", {
      signal: abortController.signal,
    });
    return response.data;
  }
  async function fetchUserColumnPreferences(abortController) {
    const response = await axiosInstance.get(
      "/objects/assets/column-visibilities",
      { signal: abortController.signal },
    );
    return response.data;
  }
  const fetchData = useCallback(
    async (abortController) => {
      const assetsPromise = fetchAssetsByPageAndLimit(
        abortController,
        page,
        pageLimit,
      );
  ...
  const refreshData = useCallback(async () => {
    ...
    fetchAssetsByPageAndLimit(abortController, page, pageLimit)
      .then((response) => {
  ...
  const setItemSerialNumbersToBeDeleted = () => {
    ...
    setOperationInfo({
      ...
      message: `Are you sure you want to delete ${selectedRowsInfo.length} assets?`,
  ...
  const setItemSerialNumbersToBeUnassigned = () => {
    ...
    setOperationInfo({
      ...
      message: `Are you sure you want to unassign ${selectedRowsInfo.length} assets?`,
      selectedRow: selectedRowsInfo,
    });
  };
  ...
  const setEmployeeIdsWithSelectedRows = (assignmentInfo) => {
    ...
    setOperationInfo({
      ...
      message: `Are you sure you want to assign ${bulkAssignInfo.selectedRows.length} assets?`,
      ...
      >
        ...
        <DialogContent>
          ...
          {operationInfo.operation == "assign_bulk" && (
            <Box sx={{ mt: 2 }}>
              {operationInfo.selectedRow.length} assets will be assigned to the
              selected employee. Are you sure you want to assign these assets?
            </Box>
          )}
        </DialogContent>
        <DialogActions>

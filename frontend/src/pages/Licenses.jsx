...
const LicensePage = () => {
  ...
  const [bulkAssignInfo, setBulkAssignInfo] = useState({
    ...
    selectedRows: [],
    serialNumbersMappedWithEmployeeIds: [],
  });
  const [filters, setFilters] = useState(
    sessionStorage.getItem("licenses-filters")
      ? JSON.parse(sessionStorage.getItem("licenses-filters"))
      : null,
  );
  //Fetching Data Functions
  const fetchLicensesByPageAndLimit = useCallback(
    async (abortController, page, limit) => {
      const response = await axiosInstance.get(
        "/objects/licenses/filter-docs/paginate",
        {
          params: {
            page: page,
            limit: limit,
  ...
  async function fetchLicensesMetaData(abortController) {
    const response = await axiosInstance.get("/metadata/licenses", {
      signal: abortController.signal,
    });
    return response.data;
  }
  async function fetchUserColumnPreferences(abortController) {
    const response = await axiosInstance.get(
      "/objects/licenses/column-visibilities",
      { signal: abortController.signal },
    );
    return response.data;
  }
  const fetchData = useCallback(
    async (abortController) => {
      const licensesPromise = fetchLicensesByPageAndLimit(
        abortController,
        page,
        pageLimit,
      );
  ...
  const refreshData = useCallback(async () => {
    ...
    fetchLicensesByPageAndLimit(abortController, page, pageLimit)
      .then((response) => {
  ...
  async function saveEditedData(formData) {
    try {
      ...
      if (response.data.success) {
        ...
        setEditInfo((prev) => {
          return { ...prev, showEditForm: false };
        });
      }
    } catch (error) {
      toast.error(error.response.data.message || "An error occurred");
  ...
  const setAssignRowIndex = (rowIndex) => {
    setOperationInfo({
      showSnackBar: true,
      operation: "assign",
      message: `Are you sure you want to assign asset ${data.data.documents[rowIndex].serial_no} to an employee?`,
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setOperationInfo((prev) => {
          return { ...prev, showSnackBar: false };
        });
      }
    } catch (error) {
      const errorMessage =
        error.response?.data?.message || "Error completing checkout";
  ...
  const setUnAssignRowIndex = (rowIndex) => {
    setOperationInfo({
      showSnackBar: true,
      operation: "unassign",
      message: `Are you sure you want to unassign asset ${data.data.documents[rowIndex].serial_no} from ${data.data.documents[rowIndex].assigned_to}?`,
      selectedRow: rowIndex,
    });
  };
  // Unassign an asset
  async function unAssignAsset() {
    try {
      const response = await axiosInstance.post(
        `/checkout/unassign/individual`,
        {
          object_name: "licenses",
          info_to_unassign: {
            serial_no: data.data.documents[operationInfo.selectedRow].serial_no,
            employee_id:
              data.data.documents[operationInfo.selectedRow].assigned_to,
          },
        },
      );
      if (response.data.success) {
  ...
  const setDeleteRowIndex = (rowIndex) => {
    ...
    setOperationInfo({
      showSnackBar: true,
      operation: "delete",
      message: `Are you sure you want to delete ${data.data.documents[rowIndex].serial_no} asset?`,
      selectedRow: rowIndex,
    });
  };
  ...
  const setItemSerialNumbersToBeDeleted = () => {
    ...
    setOperationInfo({
      ...
      message: `Are you sure you want to delete ${selectedRowsInfo.length} licenses?`,
      selectedRow: selectedRowsInfo,
    });
  };
  ...
  const setItemSerialNumbersToBeUnassigned = () => {
    ...
    setOperationInfo({
      showSnackBar: true,
      operation: "unassign_bulk",
      message: `Are you sure you want to unassign ${selectedRowsInfo.length} licenses?`,
      selectedRow: selectedRowsInfo,
    });
  };
  ...
  const setEmployeeIdsWithSelectedRows = (assignmentInfo) => {
    ...
    setOperationInfo({
      showSnackBar: true,
      operation: "assign_bulk",
      message: `Are you sure you want to assign ${bulkAssignInfo.selectedRows.length} licenses?`,
  ...
  async function searchText(value) {
    ...
    const matchesExisting = data.data.documents.some(
      (asset) =>
  ...
  const employeeOptionLabel = (option) => {
    return option
      ? `${option.employee_id || ""} ${option.firstname || ""} ${option.lastname || ""}`.trim()
      : "";
  };
  const employeeOptionEqualToLabel = (option, value) => {
  ...
  return (
    <React.Fragment>
        ...
        <React.Fragment>
          ...
          {editInfo.showEditForm && data.fields && data.data && (
            <EditForm
              ...
              closeDialog={() => {
                setEditInfo({ selectedRow: null, showEditForm: false });
              }}
          ...
          {bulkAssignInfo.showBulkAssignDialog && data.fields && data.data && (
            <AssignMultipleItems
              ...
              closeDialog={() => {
                setBulkAssignInfo((prev) => {
                  return { ...prev, showBulkAssignDialog: false };
                });
              }}
              fields={data.fields}
      ...
      >
        <DialogTitle>
          Confirm{" "}
          {operationInfo.operation?.substring(0, 1).toUpperCase() +
            operationInfo.operation?.substring(1)}
        </DialogTitle>
        <DialogContent>
          {operationInfo.message}
          {operationInfo.operation == "unassign" && (
            <Typography variant="body2" sx={{ mt: 2, color: "text.secondary" }}>
              Unassigning an asset will make it available for reassignment. Are
              you sure you want to unassign this asset?
            </Typography>
          )}
          {operationInfo.operation == "assign" && (
            <Box sx={{ mt: 2 }}>
              <AsynchronousAutoComplete
                ...
                sendInputToParent={handleAutoCompleteChange("employee_id")}
              />
            </Box>
          )}
          {operationInfo.operation == "assign_bulk" && (
            <Box sx={{ mt: 2 }}>
              {operationInfo.selectedRow.length} licenses will be assigned to
              the selected employee. Are you sure you want to assign these
              licenses?
            </Box>
          )}
        </DialogContent>
        <DialogActions>
          ...
          <Button
            onClick={dialogActions[operationInfo.operation]}
            color={dialogActionColors[operationInfo.operation]}
          >
            {operationInfo.operation?.substring(0, 1).toUpperCase() +
              operationInfo.operation?.substring(1)}
          </Button>

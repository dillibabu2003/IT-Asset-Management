...
const getDataFromGemini=(file,fileType)=>{
    const generativeAI = new GoogleGenerativeAI(import.meta.env.VITE_GOOGLE_API_KEY);
    ...
    const schema = {
        ...
        properties: {
            ...
            invoice_type:{
                type: SchemaType.STRING,enum:["lease","own"],
                description: "If the Owner is DarwinBox give value as own otherwise lease"
            },
            invoice_description:{
                type: SchemaType.STRING,
            ...
            assets: {
                ...
                items: {
                    type: SchemaType.OBJECT,
                    properties: {
                        serial_no: { type: SchemaType.STRING, description: "Unique serial number of the asset" },
                        category: { type: SchemaType.STRING, enum: ["laptop", "desktop", "server", "printer", "monitor", "mouse", "keyboard"], description: "Category of the asset" },
                        make: { type: SchemaType.STRING,enum:['Lenovo', 'Mac', 'Dell', 'HP', 'Logitech', 'Samsung', 'LG', 'Epson'], description: "Manufacturer of the asset" },
                        model: { type: SchemaType.STRING,enum:[
                            // Lenovo models
                            'ThinkPad', 'IdeaPad', 'Legion', 'Yoga', 'ThinkCentre',
                            // Mac models
                            'MacBook Pro', 'MacBook Air', 'iMac', 'Mac Pro', 'Mac Mini',
                            // Dell models
                            'Latitude', 'Precision', 'XPS', 'Inspiron', 'OptiPlex',
                            // HP models
                            'ProBook', 'EliteBook', 'Pavilion', 'Envy', 'Spectre',
                            // Logitech models
                            'MX Master', 'M185', 'K380', 'MX Keys', 'G Pro',
                            // Samsung models
                            'Odyssey', 'ViewFinity', 'Smart Monitor', 'C24', 'C27',
                            // LG models
                            'UltraGear', 'UltraFine', 'UltraWide', 'DualUp', 'QHD',
                            // Epson models
                            'EcoTank', 'WorkForce', 'Expression', 'SureColor', 'LabelWorks'
                          ] ,description: "Model of the asset" },
                        ram: { type: SchemaType.STRING, enum: ["8gb", "16gb", "32gb", "64gb", "128gb"], description: "RAM capacity" },
                        storage: { type: SchemaType.STRING, enum: ["256gb", "512gb", "1tb"], description: "Storage capacity" },
                        processor: { type: SchemaType.STRING,enum:[
                            // Intel processors
                            'Intel i9-13900K',
                            'Intel i9-12900K',
                            'Intel i7-13700K',
        ...
        required: ["invoice_id", "invoice_date", "vendor_name","owner_name","total_amount", "assets", "licenses","invoice_description"]
    };
    
    // Generation Configuration
    const generationConfig = { temperature: 0.9, topP: 1, topK: 1, maxOutputTokens: 7000, responseMimeType: "application/json",
        responseSchema: schema,};
    
    // Initialize the model
    const model = generativeAI.getGenerativeModel({ model: "gemini-1.5-pro", generationConfig });

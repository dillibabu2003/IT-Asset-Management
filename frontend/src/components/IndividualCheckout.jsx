...
function IndividualCheckout() {
  ...
  async function fetchMetaData(object_name, abortController) {
    const response = await axiosInstance.get(`/metadata/${object_name}`, {
  ...
  useEffect(() => {
    ...
    fetchMetaData(filter.object_name, abortController)
      .then((response) => {
  ...
  const getFilteredData = async () => {
    const response = await axiosInstance.post(
      `/objects/${filter.object_name}/filter-docs/all`,
      filter,
    );
  ...
  return (
    <Box>
      ...
      {filteredData && (
        <Paper sx={{ p: 2, mb: 2 }}>
          <Typography variant="h6" gutterBottom>
            Total {filter.object_name} {filteredData.length}
          </Typography>
        </Paper>
      )}
        ...
        <React.Fragment>
          <Box sx={{ mt: 3, mb: 3 }}>
            <AsynchronousAutoComplete
              ...
              sendInputToParent={handleChange("employee_id")}
            />
          </Box>
          <Grid container spacing={3}>
            <Grid item xs={12}>

...
function InvoiceSection({ refreshData }) {
  ...
  const uploadFileToS3 = async (url, file, fileType) => {
    const s3Response = await axiosInstance.put(url, file, {
  ...
  const handleSaveInvoice = async () => {
    ...
    try {
      const invoice_filename =
        currentInvoice.invoice_id + "." + fileType.split("/")[1];
      ...
      if (preSignedUrlResponse.data.success) {
        ...
        if (response) {
          toast.success("Invoice uploaded and data saved successfully.", {
            id: toastId,
          });
      ...
      console.error(
        "Error occurred while uploading or saving data. Try again.",
        error,
      );
  ...
  return (
    <Box>
      ...
      {!currentInvoice && <React.Fragment></React.Fragment>}
    </Box>
  );
}

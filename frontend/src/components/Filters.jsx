...
const Filters = (
  ...
  const [appliedFilters, setAppliedFilters] = React.useState(sessionStorage.getItem(`${currentSection}-filters`) ? JSON.parse(sessionStorage.getItem(`${currentSection}-filters`)) : {});
  ...
  return (
    <div className="filters-container" style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      ...
      <Box sx={{ display: 'flex', gap: 1 }}>
        {Object.keys(appliedFilters).length > 0 && 
          Object.entries(appliedFilters).map(([key, value]) => {
            ...
            const optionLabel = column?.options?.find(opt => 
              (typeof opt === 'object' ? opt.value : opt) === value
            );
            const displayValue = typeof optionLabel === 'object' 
              ? optionLabel.label 
              : (optionLabel || value);
      ...
      <Dialog open={open} onClose={handleCloseDialog} maxWidth="sm" fullWidth>
              ...
              {selectColumns.map(column => {
                ...
                return (
                  <FormControl key={columnKey} fullWidth margin="dense">
                    <InputLabel id={`filter-label-${columnKey}`}>{columnName}</InputLabel>
                    <Select
        ...
        <DialogActions>
          <Button onClick={handleResetFilters} color="error">Reset</Button>

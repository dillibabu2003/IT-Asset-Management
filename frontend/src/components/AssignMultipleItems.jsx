...
const AssignMultipleItems = ({
  ...
  const handleAutoCompleteChange = (serial_no) => (event) => {
    ...
    setBulkAssignInfo([
      ...bulkAssignInfo,
      { serial_no: serial_no, employee_id: event.target.value.employee_id },
    ]);
  ...
  return (
    <Dialog open={isDialogOpen} onClose={closeDialog} maxWidth="md" fullWidth>
      <DialogTitle>
        Assign{" "}
        {currentSection.substring(0, 1).toUpperCase() +
      ...
      <DialogContent>
        ...
        {selectedItemsSerialNumbers.length == 0 ? (
                ...
                {items.map(
                  (item) =>
                    ...
                    Object.entries(item).map(([key, value]) =>
                      ["__v", "employee_name"].includes(key) ||
                      key.match(/^[a-zA-Z]*_id$/) ? null : (
                        <Typography key={`${serial_no}-${key}`}>
                          {convertSnakeCaseToPascaleCase(key)}: {value}
                        </Typography>
                      ),
                    ),
                )}
